...
def is_high_risk_mod_warning(message: str) -> bool:
    """判断消息是否为高风险模组资源告警。"""
    return message.startswith(HIGH_RISK_MOD_WARNING_PREFIX)


def format_high_risk_mod_warning(message: str) -> str:
    """生成带实机后果和处理建议的固定危险资源块。"""
    details = message.removeprefix(HIGH_RISK_MOD_WARNING_PREFIX).strip()
    border = "=" * CONSOLE_ALERT_WIDTH
    separator = "-" * CONSOLE_ALERT_WIDTH
    return "\n".join(
        (
            border,
            "[!!!] HIGH-RISK MOD RESOURCE / 高风险模组资源",
            separator,
            details,
            separator,
            "[!!!] 本提示不会自动禁用模组；若未经当前版本实机验证，建议先停用。",
            border,
        )
    )


def print_high_risk_mod_warning(message: str, stream: TextIO | None = None) -> None:
