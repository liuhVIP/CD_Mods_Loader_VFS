...
英文残留；动态物品名、分类名与仓库名通过 Hook 名称 getter（0x22250）按游戏表行号查中文回退表。
物品使用 ItemInfo 定义 +0x20 的 LocString（表全局 0x179B00），分类使用 ItemGroupInfo 定义
+0x18（表全局 0x179B08），仓库使用 InventoryInfo 定义 +0x70（表全局 0x179B18）。0x22250 的
19 字节序言含一条 RIP 相对 cmp（检查名称池全局 0x179B20），跳板复制后会重定位该 disp32。
